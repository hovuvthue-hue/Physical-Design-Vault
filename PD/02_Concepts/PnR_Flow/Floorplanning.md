...
**Floorplan Quality Check — 6 Categories (sub-step 6):**

| Category | Nội dung chính |
|---|---|
| 1. Power Grid Integrity | Power Connectivity (SC rows → M1 PG rails; Macros/Pads → PG mesh); PG DRC (Tap Cell insertion nếu tapless library, floating PG shapes, shorts, spacing errors); PG Stripe/Ring Coverage (verify coverage toàn core, check gaps near macros/IOs). Early EMIR: ước tính IR Drop và EM trước Routing qua `analyze_rail`. |
| 2. IO Pads/Pins & Boundary Cells | IO Pad placement hợp lệ trên padframe; boundary cells đầy đủ tại biên core. |
| 3. Macro Placement | Macro positions đúng chiến lược; FIXED status; halos/keep-out đủ; không overlap với rows. |
| 4. Placement Rows & Core Utilization | Rows không bị bisect bởi Macro; core utilization trong target range; Row coverage đầy đủ. |
| 5. Blockage Planning | Placement Blockages và Routing Blockages đúng loại, đúng vị trí; không tạo isolated SC areas. |
| 6. General Sanity Checks | `check_floorplan`: routing tracks, FINFET/Manufacturing Grid, core/die snap, Row symmetry, AreaIO rows, pre-routes, constraints (guide/region/fence). |
