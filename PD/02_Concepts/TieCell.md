...
Direct gate-to-VDD/VSS connection có thể làm tăng rủi ro stress trên gate oxide khi có antenna charge trong quá trình chế tạo hoặc voltage surge khi vận hành, nên thường không được xem là thực hành an toàn trong physical implementation. Chi tiết cơ chế device-level phụ thuộc library/process. **[Needs verification]**
...
Trong nhiều flow, tie-off insertion diễn ra sau khi đã có placement hợp lệ hoặc trong các vòng post-placement optimization; thời điểm chính xác phụ thuộc tool flow/library setup. **[Needs verification]**

TieCell cũng thường được dùng để tie input của [[SpareCell]] về known state trước tape-out để tránh floating inputs. **[Needs verification]**
