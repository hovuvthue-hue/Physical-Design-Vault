...
## Definition
Metal Stack là toàn bộ hệ thống các lớp kim loại dẫn điện (metal layers) xếp chồng lên nhau theo chiều thẳng đứng trong một IC, được ngăn cách bởi các lớp điện môi (dielectric), và được kết nối với nhau qua các lỗ Via. Metal Stack là không gian vật lý ba chiều mà toàn bộ signal routing, power distribution, và clock delivery xảy ra. Foundry định nghĩa Metal Stack cho từng process node — số lượng layers, thickness, resistivity, và dielectric properties của mỗi layer là fixed constraints mà PD engineer không thể thay đổi.

## Computed from
Metal Stack được Foundry phát triển trong quá trình technology node development và cung cấp cho khách hàng qua Tech LEF và ITF files. Cấu trúc điển hình của một Metal Stack (TSMC 22nm 5x1z1u làm ví dụ từ slide):
...
TSMC naming convention cho Metal Stack:
...
Tên stack "5x1z1u" = 5 standard layers + 1 z-layer + 1 u-layer (không count M1 local layer).
...
- **[[ParasiticExtraction]]**: thickness, resistivity, và dielectric constants của từng layer trong Metal Stack là physical parameters cho RC extraction; layer stackup trong ITF là input trực tiếp cho extraction tool
...
- [[ITF]] — Interconnect Technology File chứa chi tiết physical parameters của Metal Stack cho RC extraction: thickness (H), dielectric constants (ε), resistivity (ρ) per layer và per corner
...
## Key insight
[USER REVIEW — draft suggestion]: Metal Stack là lý do tại sao advanced node chips tốn hàng tỷ USD để phát triển — mỗi layer đại diện cho một mask trong fab process, và ở TSMC 3nm một chip có thể cần 80+ layers (mỗi mask trị giá hàng triệu USD). Từ góc độ Physical Design, Metal Stack quyết định "routing budget" — số Track resources available trên toàn chip. Một insight quan trọng: khi chip cần thêm routing resources và không thể increase die size, giải pháp là thêm metal layers — đây là một trong những lý do advanced node chips có nhiều layers hơn (Intel 4nm có 13+ metal layers vs TSMC 180nm chỉ có 6 layers). Trade-off là cost — mỗi layer thêm vào tăng mask cost và cycle time của fab.
...
→ Affects: [[Routing]] layer assignment · [[ParasiticExtraction]] RC values · PDN in [[Floorplanning]]
