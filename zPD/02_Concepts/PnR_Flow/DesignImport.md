...
## Definition
DesignImport là bước đầu tiên của PnR flow, gồm 7 sub-steps nhằm đọc, tích hợp, và kiểm tra toàn bộ inputs trước khi chuyển sang Floorplanning. Tool chuẩn: **Cadence Innovus**. Exit criterion của DesignImport: design meet timing với zero RC delays, constraints đầy đủ, không có errors.
...
Sau bước 5 (Initialize), PnR tool hiển thị design database ở trạng thái sơ khai với ba vùng: Modules (Core logic chưa có vị trí cố định), Floorplan boundary (nếu die size đã được set), và Blocks/Blackboxes (hierarchical blocks hoặc undefined modules).
...
- Không có dropped constraints (SDC objects không resolve → tool silently drop)
...
**Bước 7 — Quick Timing Checks** — lý thuyết đằng sau mỗi loại check:
...
với $t_{logic,max} = \sum \text{CellDelay}_i$ (không có $\text{NetDelay}$). Negative zero-RC Slack = vấn đề Synthesis, phải fix trước khi tiếp tục PnR.
