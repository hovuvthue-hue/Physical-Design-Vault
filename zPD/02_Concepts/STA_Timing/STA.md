...
STA chạy tại nhiều thời điểm trong PD flow với độ chính xác tăng dần: pre-layout (WLM → ước tính), post-placement (RC estimate từ wire length), post-CTS (propagated clock, real Skew), post-route (full SPEF → chính xác nhất).
...
- **[[ClockTreeSynthesis]]**: STA chạy lại sau CTS với propagated clock (real Skew thay vì ideal); lần đầu tiên Hold violations xuất hiện do [[ClockSkew]] — đây là trigger để insert Hold fix buffers
...
- [[ClockTreeSynthesis]] — STA sau CTS (post-CTS STA) là milestone quan trọng: lần đầu tiên dùng propagated clock với real Skew và Insertion Delay thay vì ideal clock; Hold violations phát sinh ở đây cần fix trước Routing
